 ...
 * ?phase=chewy-heal RE-ATTACHES Chewy prices to products left with no price row
 * (they render as "$NaN"). Resumable — just hit it again if it defers any.
 *   e.g.  https://<app>.vercel.app/api/seed-products?secret=<SEED_SECRET>&phase=chewy-heal
 *
...
const { importChewy, seedChewyExtras, healChewyPrices } = require('../scripts/import-impact-chewy.js');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    } else if (phase === 'chewy-heal') {
      // Re-attach Chewy prices to products that ended up with no price row
      // (they were rendering as "$NaN").
      out.chewy_heal = await healChewyPrices({ log: (m) => log.push(m) });
    } else {
      if (phase !== 'stores') {
